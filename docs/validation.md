...
External-reading and current Power BI update on 2026-09-24: the read-only [external check](external-water-validation.md) processed 573,605 source observations from two published households and reported coverage, usable daily intervals and rule burden without claiming incident detection scores. The current four-page Power BI model passed native Power Query refresh and DAX/CSV parity for 18 measures, seven tables, four districts and 90 dates using the sole running Desktop Store engine; [evidence](powerbi-validation-2026-09-24.json) includes exact values and input hashes. PBIP opening and fourth-page visual rendering remain unverified. The local Python suite passed 42 tests; Ruff and repository checks passed.

Ingestion recovery update on 2026-09-24: the local SQLite suite passed 40 Python tests, including a failed transaction retried with the same hash, recovery of an abandoned `running` claim, rejection of an active claim and HTTP 409 behavior. Ruff lint/format, JavaScript syntax and the Edge browser regression passed; the browser checked the new import-health display before and after a daily import with no page errors. [Pull request #14](https://github.com/Moundirhzr2/aquawatch/pull/14) passed its Windows, Ubuntu, PostgreSQL and container jobs in [GitHub Actions run 36026675633](https://github.com/Moundirhzr2/aquawatch/actions/runs/36026675633); SonarCloud and Semgrep also passed. The fourth Power BI page remains unverified in Desktop.
...
The setup encountered third-party deprecation notices in the FastAPI/Starlette test-client dependencies; tests still passed. No production readiness, labeled field detection performance or financial benefit is claimed. The external check is limited to unlabeled data compatibility and rule burden.
