...
(async () => {
  ...
  await page.getByText('2 volume differences', {exact:false}).waitFor();
  assert.equal(await page.locator('#billing-reconciliation tbody tr').count(), 10);
  await page.locator('#billing-reconciliation tr').filter({hasText:'INV-0029'}).locator('[data-case]').click();
  await page.getByRole('dialog', {name:'Invoice volume differs from meter readings'}).waitFor();
  await page.screenshot({path:path.join(images, 'reconciliation.png')});
  await page.getByRole('button', {name:'Close investigation',exact:true}).click();
  await page.locator('[data-billing-filter="all"]').click();
  assert.equal(await page.locator('#billing-reconciliation tbody tr').count(), 120);
  await page.locator('[data-billing-filter="attention"]').click();
  ...
  await page.getByRole('heading',{name:'Import health'}).waitFor();
  await page.getByText('1 completed · 0 failed · 0 running.',{exact:false}).waitFor();
  ...
  await page.getByText('2 completed · 0 failed · 0 running.',{exact:false}).waitFor();
  ...
  await page.getByText('87.1%',{exact:true}).waitFor();
