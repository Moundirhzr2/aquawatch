...
function chart(points, {height=200, threshold=null, label="Daily consumption in cubic meters"} = {}) {
...
async function loadOverview() {
  ...
  $("#period").textContent=`01 Jun – ${new Date(data.as_of+"T12:00:00").toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}`;
  $("#network-chart").innerHTML=chart(data.daily.map(r=>({date:r.date,value:r.valid_intervals?r.liters/1000:null})),{label:"Network daily consumption in m³, excluding invalid intervals"});
  const max=Math.max(1,...data.kinds.map(r=>r.count));
  $("#kind-breakdown").innerHTML=data.kinds.map(r=>`<div class="kind-row"><div class="kind-row-label"><span>${esc(names[r.kind])}</span><strong>${r.count}</strong></div><div class="bar-track"><div class="bar-fill" style="width:${r.count/max*100}%"></div></div></div>`).join("");
...
document.addEventListener("click",event=>{const caseButton=event.target.closest("[data-case]"),runButton=event.target.closest("[data-run]");if(caseButton)showCase(caseButton.dataset.case).catch(e=>toast(e.message,true));if(runButton)showRun(runButton.dataset.run).catch(e=>toast(e.message,true));});
